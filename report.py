        ...
        for index, vlan in zip(vlanindex, vlans):
            ...
            if vname != VL:
                for i in range(len(values)):
                    ...
                    print('vlan.combine:', combine)
                    orghex = ''
                    listvls = list(combine)
                for i in range(len(listvls)):
                    if listvls[i] == '1':
                        num = i + 1
                        print('vlan.vlanname:', str(vname) + '(' + vnum + ')')
                        print('vlan.vlansnum:', num)
                        vlanname.append(str(vname) + '(' + vnum + ')')
                        vlansnum.append(num)
        print('vlan.vlansnum.count:', len(vlansnum))
